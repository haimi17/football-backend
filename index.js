// index.js – backend API-FOOTBALL + Odds API pentru Football Pro Analyzer
...
if (!ODDS_API_KEY) {
  console.warn("INFO: ODDS_API_KEY nu este setată, layer-ul de cote automate va fi dezactivat.");
}
...
// test cheie The Odds API
app.get("/api/test-odds", async (req, res) => {
  if (!ODDS_API_KEY) {
    return res.json({
      ok: false,
      status: "NO_KEY",
      message: "ODDS_API_KEY lipsește"
    });
  }

  try {
    const data = await oddsFetch("/sports", null);
    ...
    console.error("Eroare la /sports (Odds API):", e.message);
    res.json({
      ok: false,
      status: e.status || "ERROR",
      message: e.message
...
// helper: ia fixtures cu fereastră principală + fallback
async function fetchLeagueFixturesWithFallback(comp) {
  const today = new Date();

  // fereastra principală: azi → azi + 14 zile
  const fromMain = formatDate(today);
  const toMainDate = new Date(today);
  toMainDate.setDate(toMainDate.getDate() + 14);
  const toMain = formatDate(toMainDate);

  let fixturesData = await apiFetchWithRetry("/fixtures", {
  ...
  console.warn(
    `Nu s-au găsit fixtures în fereastra principală pentru liga ${comp.name}. Fallback la fereastră extinsă.`
  );

  // fallback: azi − 2 zile → azi + 40 zile
  const fromFallbackDate = new Date(today);
  fromFallbackDate.setDate(fromFallbackDate.getDate() - 2);
  const toFallbackDate = new Date(today);
  toFallbackDate.setDate(toFallbackDate.getDate() + 40);

  const fromFallback = formatDate(fromFallbackDate);
  const toFallback = formatDate(toFallbackDate);

  fixturesData = await apiFetchWithRetry("/fixtures", {
    league: comp.apiLeagueId,
    season: comp.season,
    from: fromFallback,
    to: toFallback
...
app.get("/api/matches", async (req, res) => {
  ...
  try {
    const fixtures = await fetchLeagueFixturesWithFallback(comp);
