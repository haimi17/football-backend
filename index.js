...
async function getFixturesForCompetition(comp) {
  try {
    const d = await apiFetchWithRetry("/fixtures", {
  ...
  if (home && away) {
    try {
      ...
      if (stH && stA) {
        ...
        ctx.dataQuality =
          ctx.homeMatchesTotal >= 5 && ctx.awayMatchesTotal >= 5
            ? 1
            : ctx.homeMatchesTotal >= 3 && ctx.awayMatchesTotal >= 3
            ? 0.7
            : 0.4;
        ...
        ctx.recentFactor =
          fH.matches >= 5 && fA.matches >= 5
            ? 1
            : fH.matches >= 3 && fA.matches >= 3
            ? 0.7
            : fH.matches >= 1 && fA.matches >= 1
            ? 0.5
            : 0.3;
      }
    } catch (e) {
